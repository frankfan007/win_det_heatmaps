...
class facade(IMDB):
    ...
    def parse_json_annotation(self, annotation_path, im_path, shape):
        ...
        if "EvalDataset" not in annotation_path:
            assert label['imagePath'] == os.path.basename(im_path), \
                "Wrong Annotation<->Image correspond, %s" % annotation_path # NOTE: no execute when parsing EvalDataset
    ...
    def parse_gt_file(self, data_root_path):
        ...
        images_path_list = glob.glob(data_root_path + '/*.jpg')
        images_path_list += glob.glob(data_root_path + '/*.png')
        ...
        for s_img_path in images_path_list:
            ...
            if self.use_xml:
                annotation_path = s_img_path.rsplit('.', 1)[0] + '.xml'
                the_window_anno = self.parse_xml_annotation(annotation_path, s_img_path, shape)
            else:
                annotation_path = s_img_path.rsplit('.', 1)[0] + '.json'
    ...
    def gt_db(self):
        cache_file = os.path.join(self.cache_path, self.name + '_window-corner_db.pkl')
        ...
        img_list, window_anno_list, shape_list = self.parse_gt_file(os.path.join(self.dataset_path, 'images'))
        ...
        for n_img in range(len(img_list)):
            ...
            #TODO: generate mask NOTE: not necessarily helpful
