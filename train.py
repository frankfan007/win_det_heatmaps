...
# matplotlib.use('Agg') # comment when running on server
...
def main():
    ...
    train_imdbs = []
    valid_imdbs = []
    for n_db in range(len(config.dataset.benchmark)):
        if n_db != et:
            train_imdbs += [facade(config.dataset.benchmark[n_db], 'train',config.dataset.path[n_db])]
        else:
            valid_imdbs += [facade(config.dataset.benchmark[n_db], 'valid', config.dataset.path[n_db])]
    ...
    for epoch in range(config.train.begin_epoch, config.train.end_epoch + 1):
        ...
        beginT = time.time()
        if epoch > config.train.end_epoch - 3: #only eval late model, because eval takes too much time
        ...
        scheduler.step() # need to place scheduler.step() after optimizer.step() in version>pytorch1.4
