...
def update_config_from_args(config, args):
    ...
    config.dataset.path = [0] * len(config.dataset.benchmark) #init
    for i in range(len(config.dataset.benchmark)):
        config.dataset.path[i] = os.path.join(args.data, config.dataset.benchmark[i])
